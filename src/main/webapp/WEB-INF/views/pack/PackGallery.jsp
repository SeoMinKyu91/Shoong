	...
	<div class="container">
		...
		<div class="row">
			<c:if test="${empty packGalleryList }" var="isEmpty">
				...
				<img src="<c:url value='/change/img/elements/a2.jpg'/>" class="img-thumbnail">
				<img src="<c:url value='/change/img/elements/d.jpg'/>" class="img-thumbnail">
			</c:if>
			<c:if test="${!isEmpty}">
				...
				<div class="modal-footer">
					<form method="post" action="<c:url value="/pack/pictureInput.do"/>">
						<input class="form-control" name="imgArry" id="imgArry"
							type="hidden">
						<button type="submit" class="addImageBtn btn">저장</button>
					</form>
					<button type="button" class="addImageBtn btn" data-dismiss="modal">닫기</button>
				</div>
			</div>
		</div>
	</div>
