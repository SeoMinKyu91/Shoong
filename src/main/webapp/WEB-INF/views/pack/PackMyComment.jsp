...
<script src="https://kit.fontawesome.com/4f2219bca6.js"
	...
	<div class="row">
		<div class="col-xs-12 col-lg-12" style="padding-bottom: 20px">
			<div class="">
				...
				<a class="packMainBtn btn" href="<c:url value="/pack/comment.do"/>">ALL
					LIST</a> <a class="packMainBtn btn"
					href="<c:url value="/pack/myComment.do"/>">MY LIST</a> <a
					class="packMainBtn btn" id="btn-feed-write"
					style="margine-right: 20px;" href="">WRITE</a>
						...
						<c:if test="${not isEmpty}">
							<c:forEach items="${commentList}" var="item" varStatus="loop">
								<c:if test="${not empty item }">
									<c:forEach items="${item.packCommentImages }" var="image"
										varStatus="loop" begin="0" end="0">
										<div class="col-md-4 feed-img">
											<img alt="기본사진" src="/fileupload/${image }" /> <a
												href="<c:url value="/pack/comment/view.do?packCommentNo=${item.packCommentNo }"/>">
												<div class="feed-img-hover">
													<img
														src="<c:url value='/images/fa-icons/comment-white.png'/>"
														style="width: 25px; height: 25px;"> <span>${item.packCommentReplyCount }</span>
													<img
														src="<c:url value='/images/fa-icons/heart-white.png'/>"
														style="width: 25px; height: 25px;"> <span>${item.packCommentLikeCount }</span>
												</div>
											</a>
										</div>
									</c:forEach>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="" class="" id="feed-write-form" method="post"
					enctype="multipart/form-data">
					<div class="feed-story" contenteditable="true">회원님의 공유할 이야기를
						적어주세요</div>
					<div class="feed-img-modal">
						<span class="feed-img-modal-span">Drag & Drop Files Here</span>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<form method="post" action="<c:url value="/pack/comment/write.do"/>"
					id="sendToServerForm">
					<input type="hidden" name="imgArray" id="imgArray"
						class="form-control"> <input type="hidden"
						name='packCommentContent' id="feed-story-modal">
				</form>
				<button type="submit" class="btn btn-default" id="btnCommentWrite">Save</button>
...
<!-- 피드 글 수정 모달 시작 -->
<div class="modal fade" id="feed-update-modal" data-backdrop="false">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header" style="text-align: center;">
			...
			<div class="modal-body">
				<form action="" class="" id="feed-update-form" method="post"
					enctype="multipart/form-data">
					<div class="feed-story" contenteditable="true">회원님의 공유할 이야기를
						적어주세요</div>
					<div class="feed-img-modal">
			...
			<div class="modal-footer">
				<form method="post"
					...
					<input type="hidden" name="imgArray" id="imgArrayUpdate"
						class="form-control"> <input type="hidden"
						name='packCommentContent' class="form=control"
						id="feed-story-update-modal"> <input type="hidden"
						name="packCommentNo" id="packCommentUpdateNo">
				</form>
				<button type="submit" class="btn btn-default" id="btnCommentUpdate">Save</button>
				<button type="button" class="btn btn-default closeButton"
					data-dismiss="modal" id="feed-write-cancle">Close</button>
			</div>
		</div>
...
<script>
	$(function() {
		...
		$('#btnCommentUpdate').click(
				function() {
					console.log("수정하고 난뒤 packContent값:"
							+ $('.feed-story').text());
					$('#feed-story-update-modal').val($('.feed-story').text());

					console.log('내가 확인하고싶은 content:'
							+ $('#feed-story-update-modal').val());
					console.log($('#feed-story-modal').val());
					console.log($('#imgArray').val());
